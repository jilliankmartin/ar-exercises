...
@mens_stores.each { |n| puts "Store name: #{n.name}; Store annual Revenue: #{n.annual_revenue}."}

@womens_stores = Store.where("annual_revenue <= ?", 1000000).where(womens_apparel: true)


@womens_stores.each { |n| puts "Store name: #{n.name}; Store annual Revenue: #{n.annual_revenue}."}
